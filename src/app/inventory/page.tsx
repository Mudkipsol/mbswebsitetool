...
export default function InventoryPage() {
  ...
  const getCurrentViewData = () => {
    ...
    if (category.hasSubcategories) {
      ...
      if (!currentProduct) {
        const productList = products[currentBrand as keyof typeof products]?.[currentCategory as keyof typeof products[keyof typeof products]] || [];
  ...
  const handleAddToCart = (productData: { id: string; name: string; price: number; image: string }) => {
    const quantity = quantities[productData.id] || 0.1;
    addToCart({
    ...
    setQuantities(prev => ({ ...prev, [productData.id]: 0.1 }));
  ...
  const handleQuantityChange = (productId: string, value: string) => {
    ...
    setQuantities(prev => ({
      ...prev,
      [productId]: Math.max(0.1, numValue)
    }));
  };
                          ...
                          <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                              ...
                              <Input
                                ...
                                min="0.1"
                                step="0.1"
                                value={quantities[selectedColor] || 0.1}
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              ...
                              <Input
                                ...
                                step="0.1"
                                max={product.stock}
                                value={quantities[product.id] || 0.1}
